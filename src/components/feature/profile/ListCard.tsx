...
export const ListCard = ({ title, icon, iconClass, onSeeAll, loading, empty, emptyText, children }: ListCardProps) => {
    // Sem itens não há o que "ver tudo" — o botão só aparece quando a lista tem conteúdo
    const showSeeAll = !loading && !empty;
    ...
    return (
        <div className="flex flex-col gap-4 rounded-3xl bg-white p-5 sm:p-6">
            <div className="flex items-center justify-between">
                ...
                {/* No mobile o botão desce para o fim da lista (ver abaixo) */}
                {showSeeAll && <div className="hidden lg:block">{seeAllButton}</div>}
            </div>
            ...
            {/* Abaixo do último sinal — só no mobile, onde o cabeçalho fica apertado */}
            {showSeeAll && (
                <div className="flex justify-center border-t border-cloud-100 pt-3 lg:hidden">
                    {seeAllButton}
                </div>
            )}
        </div>
